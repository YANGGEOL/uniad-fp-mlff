...
class UniAD(nn.Module):
    ...
    def add_jitter(self, feature_tokens, scale, prob):
        ...
        if random.randint(0, 100) % 3 == 0:
            ...
            noise_vector = Uniform(-0.3, 0.3).sample(feature_tokens.shape[1:]).to(feature_tokens.device).unsqueeze(0)
            feature_tokens =feature_tokens.mul(noise_vector) + feature_tokens
        else:
            attention = torch.mean(feature_tokens, dim=1, keepdim=True)
            max_val, _ = torch.max(attention.view(feature_tokens.size(0), -1), dim=1, keepdim=True)
            threshold = max_val * np.random.uniform(0.7, 0.9)
            threshold = threshold.view(feature_tokens.size(0), 1, 1)
            drop_mask = (attention < threshold).float()
            feature_tokens = feature_tokens.mul(drop_mask)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            out_concat = out_concat + output

        if self.norm is not None:
            out_concat = self.norm(out_concat)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.norm is not None:
            output_concat = self.norm(output_concat)
            if self.return_intermediate:
                intermediate.pop()
                intermediate.append(output_concat)
